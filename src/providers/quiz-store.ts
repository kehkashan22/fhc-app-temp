...
export class QuizStoreProvider {
  ...
  addToQuizCollection(quizStore: QuizStore) {
    ...
    this.storage
      .set(userId + "/quizCollection", this.quizCollection)
      .then(data => {
  ...
  private addToFirebase(quizStore: QuizStore) {
    ...
    return firebase
      ...
      .push({
        ...
        chapterType: quizStore.chapterType,
        nature: quizStore.quiz.nature,
  ...
  removefromQuizCollection(quizStore: QuizStore) {
    const position = this.quizCollection.findIndex((quizStoreEl: QuizStore) => {
      return (
        quizStoreEl.quiz.quizId.toLowerCase() ===
        quizStore.quiz.quizId.toLowerCase()
      );
    });
  ...
  getQuizByChapter(chapterId: string): QuizStore[] {
    ...
    for (let i = 0; i < this.quizCollection.length; i++) {
      if (
        this.quizCollection[i].chapterId.toLowerCase() ===
  ...
  getQuizBySubject(subjectId: string): QuizStore[] {
    ...
    for (let i = 0; i < this.quizCollection.length; i++) {
      if (
        this.quizCollection[i].subjectId.toLowerCase() ===
  ...
  getSolvedQuizFromStore(quizStore: QuizStore) {
    //one
    let quizElement: QuizStore = this.quizCollection.find(
      (quizEl: QuizStore) => {
  ...
  isQuizStoreSolved(quizStore: QuizStore) {
    //one
  ...
  loadSolvedQuizCollection() {
    const userId = this._auth.getActiveUser().uid;
    this.storage
      .get(userId + "/quizCollection")
      .then((quizStore: QuizStore[]) => {
